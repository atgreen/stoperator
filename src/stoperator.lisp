...
;; Top level for stoperator

(markup:enable-reader)

(in-package :stoperator)
...
(defun initialize-metrics ()
  (unless *stoperator-registry*
    (setf *stoperator-registry* (prom:make-registry))
    (let ((prom:*default-registry* *stoperator-registry*))
...
(defparameter *stoperator-registry* nil)
...
(markup:deftag page-template (children &key title)
   <html>
     <head>
       ...
       <link rel="stylesheet" href="css/stoperator.css" />
     </head>
     <body>
     ,@(progn children)
     </body>
...
(easy-routes:defroute index ("/") ()
  (markup:write-html
   <page-template title="stoperator">
   This is the index page of my new app, version ,(progn +stoperator-version+).
   </page-template>))
...
(defun start-server (&optional (config-ini "/etc/stoperator/config.ini"))
  ...
  (bt:with-lock-held (*server-lock*)
    ...
    (log:info "Starting stoperator version ~A" +stoperator-version+)
    ...
    (flet ((get-config-value (key)
      ...
      (setf hunchentoot:*dispatch-table* +stoperator-dispatch-table+)
      (setf prom:*default-registry* *stoperator-registry*)
      (setf *print-pretty* nil)
      (setf *handler* (let ((exposer (make-instance 'exposer-acceptor :registry *stoperator-registry* :port 9101)))
